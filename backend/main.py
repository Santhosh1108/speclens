...
app = FastAPI(title="SpecLens API", version="0.2.0")
...
def product_from_payload(payload: dict) -> ProductModel:
    try:
        ...
        raise HTTPException(status_code=422, detail=f"Invalid product state: {exc}") from exc
...
=======
    return {"message": "SpecLens API is running", "version": "0.2.0"}
...
def generate_prd_endpoint(data: dict):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"PRD generation failed: {exc}") from exc
...
def critique_prd_endpoint(data: dict):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"PRD critique failed: {exc}") from exc
